...
An agent taking over as GM should:

1. Read [campaign-state.md](campaign-state.md) and the newest session record.
2. Treat only entries under “Established canon” as facts already true in play.
3. Read [rules-and-setting-primer.md](rules-and-setting-primer.md) for the
...
- [Campaign utility kit](tools/README.md): dependency-free dice, d100 Test
  resolution, and a persistent encounter dashboard.
- [Table and scene-tool assessment](table-tools.md): theater-of-the-mind
  guidance and a staged path for optional ForMander support.
- `runtime/`: ignored transient state produced during live play. Durable
  outcomes belong in campaign state and session notes instead.
